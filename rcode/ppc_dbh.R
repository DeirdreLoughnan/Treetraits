...
mcmc_intervals(postdbh_alpaForceSp) + 
  geom_vline(xintercept = mean(postdbh$muForceSp), linetype="dotted", color = "grey")  +
  theme_classic() + 
  labs(subtitle = paste0("Mean muForceSp was ", round(mean(postdbh$muForceSp),3)),
...
mcmc_intervals(postdbh_alphaChillSp) + 
  geom_vline(xintercept = mean(postdbh$muChillSp), linetype="dotted", color = "grey")  +
  theme_classic() + 
  labs(subtitle = paste0("Mean muChillSp was ", round(mean(postdbh$muChillSp),3)),
...
mcmc_intervals(postdbh_alphaPhotoSp) + 
  geom_vline(xintercept = mean(postdbh$muPhotoSp), linetype="dotted", color = "grey")  +
  theme_classic() + 
  labs(subtitle = paste0("Mean muPhotoSp was ", round(mean(postdbh$muPhotoSp),3)),
...
# require(bayesplot)
# y <- pheno$bb 
# yrep <-  postdbh[,colnames(postdbh) %in% grep( "y_hat", colnames(postdbh), value = TRUE)]
# yrepM <- colMeans(yrep)
# 
...
# dev.off()
  ...
  DBHit <- trtPheno[complete.cases(trtPheno$dbh),]
  ...
  for (ir in 1:nRepPrior){
    ...
    sigmatransect <- rtruncnorm(1, a = 0, mean = dbh.data$prior_sigma_site_mu, sd = dbh.data$prior_sigma_site_sigma)
    ...
    #general varience
    priorCheckTrait$sigmaTrait_y[priorCheckTrait$simRep == ir] <- rnorm(dbh.data$prior_sigma_traity_mu, dbh.data$prior_sigma_traity_sigma)
  ...
  png("figures/density_Trait_Prior_joint_dbh.png")
  ...
  png("figures/Mutransect_PlotPrior_joint_dbh.png")
  ...
  for (ir in 1:nRepPrior){
    ...
    muChillSp <-  rnorm(1,dbh.data$prior_sigmaChillSp_mu,dbh.data$prior_sigmaChillSp_sigma)
    ...
    #general varience
    priorCheckPheno$sigmapheno_y[priorCheckPheno$simRep == ir] <- rtruncnorm(dbh.data$prior_sigma_traity_mu,  a = 0, dbh.data$prior_sigma_traity_sigma)
