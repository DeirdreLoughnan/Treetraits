...
mu.grand <- 30 # the grand mean of the lma model
...
# dev.off()
  ...
  lma.data <- list(yTraiti = lma$lma, 
                  N = nrow(lma),
                  n_spec = length(specieslist),
                  trait_species = as.numeric(as.factor(lma$species)),
                  n_site = length(sitelist),
                  site = as.numeric(as.factor(lma$site)),
