...
df<-data.frame(slav,htv,cnv,woodv,stomv)

#Now centering the trait data **WILL BE WORKING WITH CENTERED DATA HENCEFORTH**
df_cent<-scale(df)
head(df_cent) #looks centered
#####################################################################
# Now adding separate intercepts for each species ** Starting very simple with just SLA
slac= scale(rnorm(ntot, 5, 1)) 
htc=scale(rnorm(ntot, 11, 3)) 
cnc=scale(rnorm(ntot, 10, 2))
woodc=scale(rnorm(ntot, 0.5, 0.05))
...
for (j in c(1:length(sp_mean))){
  ...
  phen_spint<- sladiff*slac+htdiff*htc+cndiff*cnc+wooddiff*woodc+stomdiff*stomc+rnorm(rep*nsite, sp_mean[j], sigma)
  temp<-data.frame(phen_spint)
  phen_cent<- rbind(fake_cent, temp)
}
# fake_spint<-vector()
...
phen_cent<-cbind(phen_cent,sp)
head(phen_cent)
###############################################
# Now combining the phenology and the trait data

fake_cent<- data.frame(phen_cent,df_cent)
head(fake_cent)
...
full<- map2stan(
  alist(
    phen_spint~ dnorm( mu , sigma_sp) ,
    mu<-a_sp[sp]+bsla*slav+bht*htv+bcn*cnv+bstom*stomv+bwood*woodv, 
...
#<><><><><><><><><><><><><><><><><>
# the estimates are much better EXCEPT for wood density and the intercepts are almost all the same! 
infprior<- map2stan(
  alist(
    phen_spint~ dnorm( mu , sigma_sp) ,
    mu<-a_sp[sp]+bsla*slav+bht*htv+bcn*cnv+bstom*stomv+bwood*woodv, 
