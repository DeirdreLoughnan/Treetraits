...
for (i in 1:Ntrt){
  trt.dat$yTraiti[i] <- trt.dat$mutranE[i] #+ trt.dat$alpha.tranlat[i] * (trt.dat$dumE[i]*trt.dat$lat[i])  
  + trt.dat$mu.trtsp[i] + trt.dat$trt.er[i] 
}
...
mdl <- stan("stan/testTraitPhenoLatitudeNoGrand.stan",
            data = all.data,
            iter = 4000, warmup = 3000, chains=4,
            include = FALSE, pars = c("y_hat")
)
...
muTranLat <- sumer[grep("mu_tran", rownames(sumer)), c("mean","2.5%","25%","50%", "75%","97.5%")]
...
#                        "2.5"= c(muTran[2], muGrand[1,2],sigma_tran[2], sigma_sp[2], sigma_traity[1]),
...
mdl.out <- data.frame( "Parameter" = c("mutran","mu_forcesp","mu_chillsp","mu_photosp","sigma_tran","sigma_forcesp","sigma_chillsp","sigma_photosp", "sigma_phenoy","betaF", "betaC", "betaP"),  
                       "Test.data.values" = c( mu.tran, mu.force, mu.chill, mu.photo,sigma.tran, sigma.force, sigma.chill, sigma.photo,sigma.pheno.y,betaTraitxForce,betaTraitxChill,betaTraitxPhoto) ,
