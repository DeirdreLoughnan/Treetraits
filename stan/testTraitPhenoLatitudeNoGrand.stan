...
transformed parameters{
  ...
  // }
  for (i in 1:N){
    y_hat[i] =  b_tranE * tranE[i] //+ b_tranlat * interTranLat[i] 
    + muSp[trait_species[i]] ;
  }
  // Phenology
  // for (isp in 1:n_spec){
...
model{
  ...
  // b_tranlat ~ normal(mu_tranlat, sigma_tranlat);
  //// priors
 // mu_grand ~ normal(10,10);
  ...
  // //// priors
  // muPhenoSp ~ normal(100,20);
  // sigmaPhenoSp ~ normal(50,50);
  // 
