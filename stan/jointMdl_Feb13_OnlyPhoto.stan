...
data {
  ...
  // real prior_sigma_site_mu;
  // real prior_sigma_site_sigma;
...
model{
  ...
  //sigma_site ~ normal(prior_sigma_site_mu, prior_sigma_site_sigma);
  sigma_traity ~ normal(prior_sigma_traity_mu, prior_sigma_traity_sigma);

  // Phenology
  //// likelihood
  for (i in 1:Nph){
    yPhenoi[i] ~ normal(alphaPhenoSp[phenology_species[i]] + betaPhotoSp[phenology_species[i]] * photoi[i], sigmapheno_y);
