...
\begin{figure}[ht]\centering\hfill
 \centering
 \includegraphics[width=\textwidth]{..//analysis/figures/cuetraitRingType_Known.pdf}
\caption{ Estimated speciese-level cue response for the subset of species with known wood architecture relative to their wood specific desnities. Each cross represents the 50\% unvertainty interval of the cue response and trait value, with colors depicting different types of wood architecture.  a) Depicts the relationship to forcing cues, b) chilling cues, c) photoperiod cues. }
