...
\item Our study = one of the first to combine trait data with data of phenological cue response for the same individuals and species assemblages across latitudes
\begin{enumerate}
\item Our data spans plant communities in eastern and western deciduous forests of North America
\item Samples collected from multiple populations---model accounts for variation across populations and transects
\item Includes woody tree and shrub species = different growth strategies---should vary in suites of traits
...
% 1. What other functional traits relate to budburst phenology in temperate woody species?
\item Found only two of our five traits were related to budburst phenology
\begin{enumerate}
\item Of the 5 commonly measured traits we measured---only height and DBH correlated with budburst cues
\item Height and DBH: short trees had stronger forcing responses (Fig. \ref{fig:traitSlopes} a and g), but weaker responses to photoperiod (Fig. \ref{fig:traitSlopes} c and i))
\item Other three traits, LMA, SSD, C:N = no relationship to chilling, forcing, or photoperiod (Fig. \ref{fig:traitSlopes}). 
\item Exemplified by the nonexistent effect of traits on phenology---estimate of our betaTraitxCue parameter is fully informed by prior not the data (Fig. \ref{fig:betaTraitxCue})
\end{enumerate}
\end{enumerate}
...
\caption{Muplots of each trait model.}
\label{fig:muPlot}
\end{figure}
