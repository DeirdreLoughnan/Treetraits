...
These plots and tables are using the model output from the model with a dummy variable for transect (east = 1) and an interaction between transect and latitude. Latitude is included as a continuous variable, but based on the prior predictive checks, I decided to z-scored it. 

\section*{Results}

\begin{enumerate}
%brief summary of what we did
\item Our study = one of the first to combine trait data with data of phenological cue response for the same individuals
\begin{enumerate}
...
\item Height and DBH: short trees had stronger forcing responses, but weaker responses to photoperiod
\item Other three traits, LMA, SSD, C:N = no relationship to chilling, forcing, or photoperiod. 
\item Exemplified by the nonexistent effect of traits on phenology---estimate of our betaTraitxCue parameter is fully informed by prior not the data
\end{enumerate}
\end{enumerate}
