...
\item Focus on angiosperm spp only---leafout likely under more similar environmental controls than gymnosperm, conifer, species---previous work (Nacho's) has shown similar cue responses with phylogeny
\end{enumerate}
\end{enumerate}
...
\item Eastern transect: traits measured summer 2015---need to look up specific dates
...
%dlDec31: agreed, it would be ideal to submit the pheno paper soon
% \begin{enumerate}
...
\item For detailed discussion of study sample collection and methods  see Loughnan et al. (2024)
\end{enumerate}
...
\item Latitude values z-scored %24Dec2023emw -- weird choice -- why? I guess okay, but be prepared to explain why ONLY this was z-scored. I would probably combine this and the below in a sentence saying you worked mainly in natural units, but made several small changes for the model numerical stability. You could ask Mike his advice on how to explain/phrase when he visits. 
...
\item Using a joint modeling approach we then combined our trait data with budburst data from \Sexpr{noIndivPheno} cuttings across the same \Sexpr{nSpp} species sampled in the field. 
\end{enumerate}
...
%dlDec20: the height result seems strange to me, that the model would estimate greater height for the west coast that has so many more shrubs, both have left skewed height distribution, but the west coast does have some very tall trees
\begin{enumerate}
\item Most of our had negligible trait variation across populations---neither between the two transects (main effect of transect only) or by latitudes within each transect (an interactive effect between transect and latitude) %24Dec2023emw  -- rewrote end less statistically -- 
\item Leaf mass area differed by latitude within transects (\Sexpr{lmaTranLat}, UI: \Sexpr{lower_lmaTran},\Sexpr{upper_lmaTran})---eastern species LMA increased with latitude---generally greater than western species
\item LNC similarly showed a weak interaction between latitude and transect (\Sexpr{lmaTranLat}, UI: \Sexpr{lower_lmaTran},\Sexpr{upper_lmaTran})---higher latitude populations in eastern transect had lower LNC than lower latitude populations and western species %24Dec2023emw  -- where is taller? And what is this weak effect? Please give all the numbers for stats you're reporting ... and please adjust the rest below so you're consistent in phrasing and giving directionality and all stats. (This also makes it hard to address your question, but be sure you are taking into account all transect and latitude effects when you try to interpret this effect. )
...
\item Across each of our models---species level differences exceeded population level variation---reference muSp? 
\item Fig. \ref{fig:tranInt}

% DBH (\Sexpr{dbhLatTran}, UI: \Sexpr{lower_dbhLatTran},\Sexpr{upper_dbhLatTran}), and LMA (\Sexpr{lmaLatTran}, UI: \Sexpr{lower_lmaLatTran},\Sexpr{upper_lmaLatTran})---strong interactions between latitude and transect
% \item Tree DBH and LMA both increased with latitude
% \item LNC differed between the two transects (\Sexpr{lncTran}, UI: \Sexpr{lower_lncTran},\Sexpr{upper_lncTran})---greater in western transect
% \item But no population-level variation in SSD
\item Fig. \ref{fig:tranInt} %24Dec2023emw -- I would end this section by contrasting population level variation versus species -- you want to remind readers that species differences DWARF population differences before you go ahead and focus on species below. 
\end{enumerate}
...
\item Regardless of trait effects, we found temperature effects to be stronger than photoperiod on budburst 
\begin{enumerate}
\item Most models estimated chilling followed by forcing as the strongest cues of budburst---but we did find unique cue responses in some trait models
\item LMA model---estimating photoperiod cues (\Sexpr{lmaMuPSpMean}, UI: \Sexpr{lower_lmaMuPSpMean}, \Sexpr{upper_lmaMuPSpMean}) that were slightly stronger than forcing cues (\Sexpr{lmaMuFSpMean}, UI: \Sexpr{lower_lmaMuFSpMean}, \Sexpr{upper_lmaMuFSpMean})---chilling effects were weak (90\% UI crossing zero) (Table \ref{tabLMA}) % (\Sexpr{lmaMuCSpMean}, UI: \Sexpr{lower_lmaMuCSpMean}, \Sexpr{upper_lmaMuCSpMean}) 
\item Estimates of photoperiod effects on budburst were also trait specific---LNC (Table \ref{tabLNC}) and SSD \ref{tabSSD} estimated weak photoperiod effects on budburst (90\% UI crossing zero)
\item 
...
\item Including trait effects provides novel insights into how they relate to phenology and cues %24Dec2023emw -- This one line reads more discussion-y, so I think you need a new topic sentence for the points below (which seem good for the results). 
\begin{enumerate}
\item Estimates bb using our model parameters show clear differences in bb timing between tree and shrub spp for some traits but not all (Fig. \ref{fig:bbShrubTree})
...
\caption{Despite species wood structures causing differing growth strategies, we did not find this trait to correlate with differences in cue responses across species. Thinner lines represent the 90\% UI and thicker lines the 50\% UI. Here we show the results for height only.}
