...
getwd()
source("..//rcode/manuscriptValues.R")
...
\item Most traits = small population effects but interaction between pop and transect for some.
\begin{enumerate}
\item Found moderate to strong interaction between latitude and transect---but effects differed by trait
\item Both height and ssd = smaller trait values at higher latitudes = but ssd showed weak effects (\Sexpr{htLatTran}, UI: \Sexpr{lower_htLatTran},\Sexpr{upper_htLatTran} for height, \Sexpr{ssdLatTran}, UI: \Sexpr{lower_ssdLatTran},\Sexpr{upper_ssdLatTran} for SSD)
...
\item Only C:N and LMA = no effect (\Sexpr{lmaLatTran}, UI: \Sexpr{lower_lmaLatTran},\Sexpr{upper_lmaLatTran} for both traits) 
...
\begin{enumerate}
\item Only height and DBH correlated with budburst cues---responses were consistent across the two traits
\item Smaller, short trees had stronger forcing responses (\Sexpr{htBFSpMean}, UI: \Sexpr{lower_htBFSpMean }, \Sexpr{upper_htBFSpMean } for height and \Sexpr{dbhBFSpMean}, UI: \Sexpr{lower_dbhBFSpMean}, \Sexpr{upper_dbhBFSpMean}, Fig. \ref{fig:traitSlopes} a and g)
...
\begin{enumerate}
\item LMA, SSD, C:N = no relationship---posterior centered around zero (Fig. \ref{fig:traitSlopes})
...
\begin{enumerate}
\item All models produced positive phenological responses to cues
...
\item Including height and DBH trait effects better informed spp level estimates of phenology
\begin{enumerate}
...
\begin{figure}[ht]\centering\hfill
 \centering
 \includegraphics[width=\textwidth]{..//figures/muCuePlots.pdf}
...
\begin{figure}[ht]\centering\hfill
 \centering
 \includegraphics[width=\textwidth]{..//figures/muCueTraitPlots.pdf}
\caption{The relationships between traits and cue responses varied considerably across each of our trait models, a. height, b. diameter at breast height, c. stem specific density, d. leaf mass area, and e. carbon:nitrogen, and for individual cues. Lines represent 90\% uncertainty intervals.}
...
\begin{figure}[ht]\centering\hfill
 \centering
 \includegraphics[width=\textwidth]{..//figures/cuetraitLatiZMdl25.pdf}
...
\begin{figure}[ht]\centering\hfill
 \centering
 \includegraphics[width=\textwidth]{..//figures/dotShrubTreeHtLMA.pdf}
...
\begin{figure}[ht]\centering\hfill
 \centering
 \includegraphics[width=\textwidth]{..//figures/betaTraitxCueZ25.pdf}
\caption{Despite differing in their wood structures and growth strategies, we did not find this trait to correlate with differences in cue responses across species.}
...
sumer <- data.frame(summary(mdl)$summary[c("mu_grand","b_tranE","b_tranlat", "muForceSp", "muChillSp", "muPhotoSp","muPhenoSp","betaTraitxForce", "betaTraitxChill","betaTraitxPhoto","sigma_traity" ,"sigma_sp", "sigmaForceSp", "sigmaChillSp", "sigmaPhotoSp","sigmaPhenoSp","sigmapheno_y"),c("mean","2.5%","25%","50%", "75%","97.5%")])
