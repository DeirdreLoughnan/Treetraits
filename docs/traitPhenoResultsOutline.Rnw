...
\begin{enumerate}
\item Most of our traits varied across populations---changing either between the two transects or with an interactive effect between transect and latitude
\item Height differed between our eastern and western transect (\Sexpr{htTran}, UI: \Sexpr{lower_htTran},\Sexpr{upper_htTran}), with a weak effect of latitude 
\item DBH (\Sexpr{dbhLatTran}, UI: \Sexpr{lower_dbhLatTran},\Sexpr{upper_dbhLatTran}), and LMA (\Sexpr{lmaLatTran}, UI: \Sexpr{lower_lmaLatTran},\Sexpr{upper_lmaLatTran})---strong interactions between latitude and transect
...
\item But no relationship between between ring porosity of wood with cues---see similar cue estimates across each of the four ring types \ref{fig:ringPorosity}.
\end{enumerate}

\item leaf traits varied more in their cue responses 
\begin{enumerate}
\item LMA showed strong responses to photoperiod with low LMA species advancing in bb with longer photoperiods (\Sexpr{lmaMuPSpMean}, UI: \Sexpr{lower_lmaMuPSpMean}, \Sexpr{upper_lmaMuPSpMean})
\item But LNC had no responses to any of our three cues
\end{enumerate}
...
\item Individual spp varied in both their timing and cue responses---but generally responses were strongest for chilling and weakest for photoperiod.
\item Only our LMA model produced unique cue estimates---estimating photoperiod cues (\Sexpr{lmaMuPSpMean}, UI: \Sexpr{lower_lmaMuPSpMean}, \Sexpr{upper_lmaMuPSpMean}) that were slightly stronger than forcing cues (\Sexpr{lmaMuFSpMean}, UI: \Sexpr{lower_lmaMuFSpMean}, \Sexpr{upper_lmaMuFSpMean}) (Table \ref{tabLMA})
...
\item Including trait effects provides novel insights into how they relate to phenology and cues
\begin{enumerate}
