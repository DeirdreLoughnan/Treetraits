...
\item Focus on angiosperm spp only---leafout likely under more similar environmental controls than gymnosperm, conifer, species---previous work has shown similar cue responses with phylogeny
\end{enumerate}
\end{enumerate}
...
\item Leaf mass area differed by latitude within transects (\Sexpr{lmaLatTran}, UI: \Sexpr{lower_lmaLatTran},\Sexpr{upper_lmaLatTran})---eastern species LMA increased with latitude---generally greater than western species---Fig. \ref{fig:tranInt} d
...
\item Traits varied in whether temperature had the strongest effects---with some traits showing stronger than expected photoperiod cues
\begin{enumerate}
