...
\begin{figure}[ht]\centering\hfill
 \centering
 \includegraphics[width=\textwidth]{..//figures/intrxnPlots100.pdf}
...
\begin{figure}[ht]\centering\hfill
 \centering
 \includegraphics[width=\textwidth]{..//figures/dotShrubTreeHtCN.pdf}
...
sum <- summary(mdlCN)$summary

fit <- rstan::extract(mdlCN)
...
print(xtable(meanz4, caption="Summary output from a joint Bayesian model of CN and budburst phenology in which species are partially pooled and ..... The model includes photoperiod and forcing as continuous variables.",label = "tabHt"),include.rownames=TRUE, caption.placement="top", hline.after=c(-1,0))
@
\newpage
